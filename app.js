(function () {
  ...
  function chipMarkup(colors, count) {
    count = count || 9;
    let out = "";
    for (let i = 0; i < count; i++) out += `<span style="background:${colors[i % colors.length]}"></span>`;
    return out;
  }
    ...
    list.forEach((p) => {
      ...
      card.innerHTML = `
        ...
        <div class="product-info">
          ...
          <span class="product-weight">${p.weight}${p.desc ? " · " + p.desc : ""}</span>
  ...
  function renderCart() {
    ...
    cartItemsEl.innerHTML = ids.map((id) => {
      ...
      return `
        <li class="cart-item">
          <div class="cart-chip-swatch">${chipMarkup(p.colors, 4)}</div>
          <div>
